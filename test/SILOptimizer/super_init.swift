...
// CHECK-NOT:     super_method [[ORIG_SELF]] : $Bar, #Foo.init!initializer.uncurried
...
class Zim: Foo {
  ...
  // CHECK-NOT:     super_method {{%[0-9]+}} : $Zim, #Foo.init!initializer.uncurried
  // CHECK:         function_ref @$s10super_init3FooCACycfC
  // CHECK:         function_ref @$s10super_init3FooCACycfc
}
...
class Zang: Foo {
  ...
  // CHECK-NOT:         super_method {{%[0-9]+}} : $Zang, #Foo.init!initializer.uncurried
  // CHECK:             function_ref @$s10super_init3FooCACycfC
  // CHECK:             function_ref @$s10super_init3FooCACycfc
}
...
class Good: Foo {
  ...
  // CHECK-NOT:     super_method {{%[0-9]+}} : $Good, #Foo.init!initializer.uncurried
  // CHECK:         [[SUPER_INIT:%.*]] = function_ref @$s10super_init3FooCyACSicfc
