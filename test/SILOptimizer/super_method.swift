...
class Grandchild : Child {
  ...
  func onlyInGrandchild() {
    // CHECK-NOT: super_method %0 : $Grandchild, #Parent.onlyInParent.uncurried : Parent -> () -> ()
  ...
  override func foo() {
    // CHECK-NOT: super_method %0 : $Grandchild, #Parent.foo.uncurried : Parent -> () -> ()
...
class ConcreteChild : GenericParent<String> {
  ...
  override init(a: String) {
    // CHECK-NOT: super_method {{%[0-9]+}} : $ConcreteChild, #GenericParent.init!initializer.uncurried
...
class ConcreteGrandchild : ConcreteChild {
  ...
  func onlyInGrandchild() {
    // CHECK-NOT: super_method {{%[0-9]+}} : $ConcreteGrandchild, #GenericParent.onlyInParent.uncurried
  ...
  override func method() {
    // CHECK-NOT: super_method {{%[0-9]+}} : $ConcreteGrandchild, #GenericParent.method.uncurried
