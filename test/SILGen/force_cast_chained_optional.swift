...
// CHECK:   class_method [[ARG]] : $Foo, #Foo.bar!getter.uncurried : (Foo) -> () -> Bar?, $@convention(method) (@guaranteed Foo) ->
...
// CHECK:   [[METHOD:%.*]] = class_method [[BORROWED_BAR]] : $Bar, #Bar.bas!getter.uncurried : (Bar) -> () -> C?, $@convention(method) (@guaranteed Bar) ->
