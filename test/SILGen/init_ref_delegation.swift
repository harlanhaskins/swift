...
class C1 {
 ...
 // CHECK-LABEL: sil hidden @$s19init_ref_delegation2C1C{{[_0-9a-zA-Z]*}}fC
  convenience init(x: X) {
    ...
    // CHECK:   [[DELEG_INIT:%[0-9]+]] = class_method [[SELF_META]] : $@thick C1.Type, #C1.init!allocator.uncurried
...
@objc class C2 {
  ...
  convenience init(x: X) {
    ...
    // CHECK:   [[DELEG_INIT:%[0-9]+]] = class_method [[SELF_META]] : $@thick C2.Type, #C2.init!allocator.uncurried
...
class C3 {
  ...
  convenience init() {
    ...
    // CHECK: class_method [[SELF:%[0-9]+]] : $@thick C3.Type, #C3.init!allocator.uncurried
    // CHECK-NOT: integer_literal
    // CHECK: return
    self.init(x: x)
  }
