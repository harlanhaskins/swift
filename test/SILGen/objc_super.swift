...
class Hoozit : Gizmo {
  ...
  override init() {
    // CHECK: objc_super_method {{%.*}} : $Hoozit, #Gizmo.init!initializer.uncurried.foreign
    super.init()
  }
  ...
  override class func runce() {
    // CHECK: objc_super_method {{%.*}} : $@thick Hoozit.Type, #Gizmo.runce.uncurried.foreign
    super.runce()
  }

  // CHECK-LABEL: sil hidden @$s10objc_super6HoozitC4frob{{[_0-9a-zA-Z]*}}F : $@convention(method) (@guaranteed Hoozit) -> ()
  override func frob() {
    // CHECK: objc_super_method {{%.*}} : $Hoozit, #Gizmo.frob.uncurried.foreign
    super.frob()
  }
}
...
class Wotsit : Hoozit {
  ...
  override func funge() {
    // CHECK: objc_super_method {{%.*}} : $Wotsit, #Gizmo.funge.uncurried.foreign
    super.funge()
  }
