...
func lazyRefPropertiesAreNotStored(_ container: LazyContainerClass) {
  ...
  // CHECK:   {{%[0-9]+}} = class_method [[ARG]] : $LazyContainerClass, #LazyContainerClass.lazyVar!getter.uncurried : (LazyContainerClass) -> () -> Int, $@convention(method) (@guaranteed LazyContainerClass) -> Int
  markUsed(container.lazyVar)
}
...
func finalVarsAreDevirtualized(_ obj: FinalPropertyClass) {
  ...
  markUsed(obj.foo)
  // CHECK: class_method [[ARG]] : $FinalPropertyClass, #FinalPropertyClass.bar!getter.uncurried
  markUsed(obj.bar)
}
