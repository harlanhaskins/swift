...
func testGizmo(gizmo: Gizmo) {
  ...
  // CHECK: objc_method [[SELF:%[0-9]+]] : $Gizmo, #Gizmo.enumerateSubGizmos.uncurried.foreign
...
func testNonnullDictionary(gizmo: Gizmo) {
  ...
  // CHECK: objc_method [[SELF:%[0-9]+]] : $Gizmo, #Gizmo.doTheThing.uncurried.foreign
  gizmo.doTheThing()
...
func testNullableDictionary(gizmo: Gizmo) {
  ...
  // CHECK: objc_method [[SELF:%[0-9]+]] : $Gizmo, #Gizmo.doTheOtherThing.uncurried.foreign
