...
func test_multiple_patterns3() {
  let f = Foo.C(0, 1, 2.0)
  switch f {
    // CHECK:   switch_enum {{%.*}} : $Foo, case #Foo.A!enumelt.uncurried: [[A:bb[0-9]+]], case #Foo.B!enumelt.uncurried: [[B:bb[0-9]+]], case #Foo.C!enumelt.uncurried: [[C:bb[0-9]+]]
...
func test_multiple_patterns4() {
  ...
  switch b {
    // CHECK:   switch_enum {{%.*}} : $Bar, case #Bar.Y!enumelt.uncurried: [[Y:bb[0-9]+]], case #Bar.Z!enumelt.uncurried: [[Z:bb[0-9]+]]
  case .Y(.A(let x, _), _), .Y(.B(_, let x), _), .Y(.C, let x), .Z(let x, _):
    // CHECK:   [[Y]]([[Y_TUP:%.*]] : @trivial $(Foo, Int)):
    // CHECK:     ([[Y_F:%.*]], [[Y_X:%.*]]) = destructure_tuple [[Y_TUP]]
    // CHECK:     switch_enum [[Y_F]] : $Foo, case #Foo.A!enumelt.uncurried: [[A:bb[0-9]+]], case #Foo.B!enumelt.uncurried: [[B:bb[0-9]+]], case #Foo.C!enumelt.uncurried: [[C:bb[0-9]+]]
