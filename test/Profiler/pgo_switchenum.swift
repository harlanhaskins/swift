...
public func guess1(x: MaybePair) -> Int32 {
  // SIL: switch_enum {{.*}} : $MaybePair, case #MaybePair.Neither!enumelt: {{.*}} !case_count(2), case #MaybePair.Left!enumelt.uncurried: {{.*}} !case_count(5001), case #MaybePair.Right!enumelt.uncurried: {{.*}} !case_count(3), case #MaybePair.Both!enumelt.uncurried: {{.*}} !case_count(5)
  // SIL-OPT: switch_enum {{.*}} : $MaybePair, case #MaybePair.Neither!enumelt: {{.*}} !case_count(2), case #MaybePair.Left!enumelt.uncurried: {{.*}} !case_count(5001), case #MaybePair.Right!enumelt.uncurried: {{.*}} !case_count(3), case #MaybePair.Both!enumelt.uncurried: {{.*}} !case_count(5)
  switch x {
  case .Neither:
    return 1
  case let .Left(val):
...
public func guess2(x: MaybePair) -> Int32 {
  // SIL: switch_enum {{.*}} : $MaybePair, case #MaybePair.Neither!enumelt: {{.*}} !case_count(2), case #MaybePair.Left!enumelt.uncurried: {{.*}} !case_count(5001), default {{.*}} !default_count(8)
  // SIL-OPT: switch_enum {{.*}} : $MaybePair, case #MaybePair.Neither!enumelt: {{.*}} !case_count(2), case #MaybePair.Left!enumelt.uncurried: {{.*}} !case_count(5001), default {{.*}} !default_count(8)
  switch x {
  case .Neither:
    return 1
  case let .Left(val):
