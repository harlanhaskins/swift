...
public func guessForEach1(x: Int32) -> Int32 {
  // SIL: switch_enum {{.*}} : $Optional<Int32>, case #Optional.some!enumelt.uncurried: {{.*}} !case_count(798), case #Optional.none!enumelt: {{.*}} !case_count(42)
...
public func guessForEach2(x: Int32) -> Int32 {
  // SIL: switch_enum {{.*}} : $Optional<(String, Int32)>, case #Optional.some!enumelt.uncurried: {{.*}} !case_count(168), case #Optional.none!enumelt: {{.*}} !case_count(42)
...
func main() {
  // SIL: switch_enum {{.*}} : $Optional<Int>, case #Optional.some!enumelt.uncurried: {{.*}} !case_count(42), case #Optional.none!enumelt: {{.*}} !case_count(1)
  var guesses : Int32 = 0;
